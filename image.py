...
class ImageDataGenerator(object):
    ...
    def __init__(self,
        ...
        self.apply_gen_transform = apply_gen_transform
    ...
    def flow_hdf5(self, x, y=None, batch_size=32, shuffle=False, seed=None,
        ...
        # Returns
            An Iterator yielding tuples of `(x, y)` where `x` is a HDF5Matrix array of image data and
             `y` is a HDF5Matrix array of corresponding labels."""
        return HDF5MatrixCacheIterator(
            x, y, self,
            batch_size=batch_size,
            shuffle=shuffle,
            seed=seed,
...
class HDF5MatrixCacheIterator(Iterator):
    """Iterator yielding data from a HDF5MatrixCache array.

    # Arguments
        x: HDF5Matrix array of input data.
        y: HDF5Matrix array of targets data.
        image_data_generator: Instance of `ImageDataGenerator`
            to use for random transformations and normalization.
        batch_size: Integer, size of a batch.
        shuffle: Boolean, whether to shuffle the data between epochs.
    ...
    def __init__(self, x, y, image_data_generator,
        ...
        if subset is not None:
            if subset not in {'training', 'validation'}:
                raise ValueError('Invalid subset name:', subset,
                                 '; expected "training" or "validation".')
            split_idx = int(x.shape[0] * image_data_generator._validation_split)
            if subset == 'validation':
                self.samples = split_idx
                self.split = 0
            elif subset == 'training':
                self.samples = x.shape[0] - split_idx
        ...
        if subset is not None:
            if (subset == 'training'):
                super(HDF5MatrixCacheIterator, self).__init__( x.shape[0] - split_idx, batch_size, shuffle, seed)
            else:
                super(HDF5MatrixCacheIterator, self).__init__( split_idx, batch_size, shuffle, seed)
        else:
