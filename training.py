...
class Model(Container):
    ...
    def evaluate_generator(self, generator, steps=None,
        ...
        return averages
    @interfaces.legacy_generator_methods_support
    def write_predict_generator(self, generator, steps=None,
                           max_queue_size=10,
                           workers=1,
                           use_multiprocessing=False,
                           verbose=0,
                           h5py_file =None,
                           h5py_label=None):
        """Extract x,y from data generator.
        ...
        try:
            ...
            x = self.predict_on_batch(x)
            batch_size = x.shape[0]
            shape = (generator.samples,)
            shape +=x.shape[1:]
            #15 Gb is upper limit for cache memory.
            total_mem_usage,dividing_factor = calculateDividingFactor(shape)
            len_tuple = len(shape)
            chunk_shape = (1,)
            for chunk_index in range(1,len_tuple):
                chunk_shape+=(max(1,shape[chunk_index]//2),)
            ...
            while steps_done < steps:
                x, y = next(output_generator)
                batch_s = steps_done * batch_size
                batch_e = min(batch_s + batch_size,generator.samples)
                d_set[batch_s:batch_e] = self.predict_on_batch(x)
                label_set[batch_s:batch_e] = y
                steps_done += 1
                if verbose == 1:
                    progbar.update(steps_done)
            ...
            if enqueuer is not None:
                enqueuer.stop()
    @interfaces.legacy_generator_methods_support
    def write_generator(self, generator, steps=None,
                           max_queue_size=10,
                           workers=1,
                           use_multiprocessing=False,
                           verbose=0,
